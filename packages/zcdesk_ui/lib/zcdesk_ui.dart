library zcdesk_ui;

// dumb_widgets exports
export 'src/widgets/zcdesk_text.dart';
export 'src/widgets/zcdesk_input_field.dart';
export 'src/widgets/zcdesk_auth_btn.dart';
export 'src/widgets/zcdesk_left_sidebar.dart';
export 'src/widgets/zc_desk_send_message_field.dart';
export 'src/widgets/zcdesk_drop_down_menu.dart';
export 'src/widgets/zcdesk_checkbox.dart';
export 'src/shared/shortCuts/short_cut_widgets.dart';

// colors export
export 'src/shared/app_colors.dart';

// shared exports
export 'src/shared/ui_helpers.dart';
export 'src/shared/svg_assets_paths.dart';
export 'src/shared/styles.dart';
